...
function theme_posts_pagination() {
    ...
    if (!empty($paginate_links)) {
        ...
        foreach($paginate_links as $k => $paginate_link) {
            $paginate_link = preg_replace_callback('/>(\d+)</', function($matches) {
                return '>' . sprintf('%02d', $matches[1]) . '<';
            }, $paginate_link);

            if ($k == 0 && strpos($paginate_link, 'prev') == false) {
                echo sprintf('<li class="cm-nav-paginate__ctrl"><a href="#" class="is-disabled">PREV</a></li>');
            }
            if (strpos($paginate_link, 'current') !== false) {
                echo sprintf('<li class="cm-nav-paginate__page"><span class="is-current">%s</span></li>', $paginate_link);
            } elseif(strpos($paginate_link, 'prev') !== false) {
                echo sprintf('<li class="cm-nav-paginate__ctrl">%s</li>', $paginate_link);
            } elseif(strpos($paginate_link, 'next') !== false) {
                echo sprintf('<li class="cm-nav-paginate__ctrl">%s</li>', $paginate_link);
            } else {
                echo sprintf('<li class="cm-nav-paginate__page">%s</li>', $paginate_link);
            }
        }
        if (strpos($paginate_link, 'next') == false) {
            echo sprintf('<li class="cm-nav-paginate__ctrl"><a href="#" class="is-disabled">NEXT</a></li>');
        }
        echo '</ul>';
    }
}
